...
handler.prototype.geojson = function(row) {
    ...
    var fields = this.getFields(row, true);
    var properties = {};
    var values = [];
    ...
    for (var i = 0, l = data.length; i < l; i++) {
        ...
        if (typeof field.value !== 'undefined') {
            ...
            values.push(display.util.ac(field.value));
        }
        else {
    ...
    properties[this.identifier + '_description'] = values.join(' / ') + ' <small>(' + this.layer.title + (this.layer.description ? ' - ' + this.layer.description : '') + ')</small>';
    return properties;
};
