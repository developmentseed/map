...
Map.prototype.json = function(layer, rows) {
    ...
    for (var l in this.map) {
        ...
        if (!layer || l === layer) {
            ...
            if (builtLayer.stylewriter && builtLayer.stylewriter.type) {
                builtLayer = this.handlers[l].json(builtLayer, rows);
            }
            // MML file path preset
            else if (builtLayer.stylewriter && builtLayer.stylewriter.url_mml) {
                var url = require('url');
                var mapPublicURL = require('expresslane').app.set('settings')('mapPublicURL');
                builtLayer.stylewriter.url_mml = url.resolve(mapPublicURL, builtLayer.stylewriter.url_mml);
            }
        ...
        for (var l in this.map) {
            ...
            if ((!layer || l === layer) && this.map[l].stylewriter) {
                ...
                for (var key in layerProperties) {
                    ...
                    properties[key.replace('.', '_')] = layerProperties[key];
                }
            }
        }
        built.features.push({
